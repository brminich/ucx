...
public:
    ...
    void create_entities(size_t alignment, size_t align_offset)
    {
        entity *sender = uct_test::create_entity(0ul, NULL, NULL, NULL, NULL,
                                                 NULL, NULL, NULL, alignment,
                                                 align_offset);
        ...
        if (UCT_DEVICE_TYPE_SELF == GetParam()->dev_type) {
            ...
            entity *receiver = uct_test::create_entity(0ul, NULL, NULL, NULL,
                                                       NULL, NULL, NULL, NULL,
    ...
    {
        size_t align       = pow(2, ucs::rand() % 13);
        ...
        if (set_offset) {
            ...
            offset = ucs_min(128, ucs::rand() % align);
        } else {
            offset = 0;
        }

        UCS_TEST_MESSAGE << "alignment: " << align << ", offset: "
                         << (set_offset ? ucs::to_string(offset) : "none");

        create_entities(align, offset);
    ...
    am_handler(void *arg, void *data, size_t length, unsigned flags)
    {
        ...
        if (flags & UCT_CB_PARAM_FLAG_DESC) {
            void *aligned_data = UCS_PTR_BYTE_OFFSET(data,
                                                     self->m_align_offset);
            EXPECT_EQ(0, ((uintptr_t)aligned_data) % self->m_alignment)
