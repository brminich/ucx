...
class test_rc_get_fc : public test_rc {
public:
    test_rc_get_fc() {
        m_num_get_ops = 8;
        if (UCS_OK != uct_config_modify(m_iface_config, "RC_TX_NUM_GET_OPS",
                                        ucs::to_string(m_num_get_ops).c_str())) {
            UCS_TEST_ABORT("Error: cannot set get fc parameters");
        }

        m_max_get_zcopy = 4096;
        if (UCS_OK != uct_config_modify(m_iface_config, "RC_MAX_GET_ZCOPY",
                                        ucs::to_string(m_max_get_zcopy).c_str())) {
            UCS_TEST_ABORT("Error: cannot set get fc parameters");
        }

        if (UCS_OK != uct_config_modify(m_iface_config, "RC_TX_CQ_LEN", "32")) {
            UCS_TEST_ABORT("Error: cannot modify RC_TX_CQ_LEN");
        }
    ...
    unsigned gets_available(entity *e) {
        return rc_iface(e)->tx.gets_available;
    }
...
UCS_TEST_P(test_rc_get_fc, get_ops_limit)
{
    check_caps_skip(UCT_IFACE_FLAG_GET_ZCOPY | UCT_IFACE_FLAG_GET_BCOPY);
    ...
    EXPECT_EQ(0, gets_available(m_e1));
    ...
    flush();
    EXPECT_EQ(m_num_get_ops, gets_available(m_e1));
}

// Check that get function fails for messages bigger than MAX_GET_ZCOPY value
UCS_TEST_P(test_rc_get_fc, get_size_limit)
{
    check_caps_skip(UCT_IFACE_FLAG_GET_ZCOPY);
...
UCS_TEST_P(test_rc_get_fc, invalid_get_size)
{
    check_caps_skip(UCT_IFACE_FLAG_GET_ZCOPY);
    ...
    if (UCS_OK != uct_config_modify(m_iface_config, "RC_MAX_GET_ZCOPY",
                                    ucs::to_string(max_ib_msg + 1).c_str())) {
        UCS_TEST_ABORT("Error: cannot modify RC_MAX_GET_ZCOPY");
    }

    scoped_log_handler wrap_err(hide_warns_logger);
    entity *e = uct_test::create_entity(0);
    m_entities.push_back(e);
...
UCS_TEST_P(test_rc_get_fc, post_get_no_res)
{
    check_caps_skip(UCT_IFACE_FLAG_GET_ZCOPY | UCT_IFACE_FLAG_AM_BCOPY);

    unsigned max_get_ops = gets_available(m_e1);
    ucs_status_t status;
    ...
    EXPECT_EQ(max_get_ops, gets_available(m_e1));
...
UCT_INSTANTIATE_RC_TEST_CASE(test_rc_get_fc)
