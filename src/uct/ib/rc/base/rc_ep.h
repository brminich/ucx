...
#define UCT_RC_CHECK_NUM_GET(_iface, _txqp) \
    if (ucs_unlikely((_iface)->tx.gets_available == 0)) { \
        UCS_STATS_UPDATE_COUNTER((_txqp)->stats, UCT_RC_TXQP_STAT_NO_GETS, 1); \
        return UCS_ERR_NO_RESOURCE; \
    }

#define UCT_RC_GET_OP_POSTED(_iface) \
    { \
        ucs_assert((_iface)->tx.gets_available > 0); \
        --(_iface)->tx.gets_available; \
    }
